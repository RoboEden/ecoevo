...
class DynamicArray(spaces.Space):
    ...
    def contains(self, x: Any) -> bool:
        if isinstance(x, list) and len(x) > self.max_length:
            return False
        elif isinstance(x, np.ndarray) and len(
                x.shape) == 2 and len(x) > self.max_length:
            return False
...
def get_obs_space(config: EnvConfig, agent_names: List[str], agent: int = 0):
    ...
    for agent_name in agent_names:
        teams_obs_space[agent_name] = spaces.Dict(
            ...
            factories_to_place=spaces.Discrete(
                config.MAX_FACTORIES +
                1),  # + 1 for potentially bidded factory
            factory_strains=DynamicArray(spaces.Discrete(max_space),
                                         max_length=(config.MAX_FACTORIES +
                                                     1)))
    ...
    board_obs_space = dict(ice=spaces.Box(low=0,
                                          ...
                                          shape=map_shape,
                                          dtype=int),
                           ore=spaces.Box(low=0,
                                          high=1,
                                          shape=map_shape,
                                          dtype=int),
                           rubble=spaces.Box(low=0,
                                             high=config.MAX_RUBBLE,
                                             shape=map_shape,
                                             dtype=int),
                           lichen=spaces.Box(low=0,
                                             high=99999,
                                             shape=map_shape,
                                             dtype=int),
                           lichen_strains=spaces.Box(low=-1,
                                                     high=99999,
                                                     shape=map_shape,
                                                     dtype=int),
                           spawns=spaces.Dict(spawns_space),
                           factories_per_team=spaces.Discrete(
                               config.MAX_FACTORIES))
