...
if __name__ == '__main__':
    ...
    if case_index == 0:
        ...
        for i in range(num_deal):
            ...
            deal = ((x, y), ('gold', -amount), ('gold', amount)) if not i % 2 else ((x, y), ('gold', -amount), ('gold',
                                                                                                                amount))
            legal_deals[i] = deal
    ...
    if trader.mode == 'ip':
        ...
        # total trade amount
        idx_item_num = 1
        total_trade_amount = sum(dict_flow[t][idx_item_num] for t in dict_flow)
        ...
        dict_deal = {
            trader.legal_deals[i][0]:
            (trader.legal_deals[i][idx_sell][idx_item_num], trader.legal_deals[i][idx_buy][idx_item_num])
            for i in trader.legal_deals
        }
        for y in range(map_size):
            for x in range(map_size):
                colour = 'red' if (x, y) in dict_deal else 'white'
        ...
        for (i, j) in dict_flow:
            ...
            idx_item_num = 1
            trade_amount = (-dict_flow[i, j][idx_item_num], dict_flow[j, i][idx_item_num])
            plt.text(x=(pos_2[0] * 2 + pos_1[0]) / 3,
                     y=(pos_2[1] * 2 + pos_1[1]) / 3,
                     s=str(trade_amount),
                     color='green')
