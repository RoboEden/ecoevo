...
def get_random_distribution(mat_type_area: np.ndarray, num_block_resource: int,
    ...
    arr_pos = np.random.choice(a=ub_x * ub_y,
                               size=num_block_resource,
                               replace=False)
    for pos in arr_pos:
...
for t in dict_type_idx.keys():
    if t == 'empty':
        ...
        mat_type_area = np.zeros(
            shape=mat_type_all[:area_size, :area_size].shape)
        get_random_distribution(mat_type_area=mat_type_area,
                                num_block_resource=num_block_resource,
                                type_item=t)
        mat_type_all[:area_size, :area_size] = mat_type_area
    elif t == 'pineapple':
        mat_type_area = np.zeros(
            shape=mat_type_all[:area_size, area_size +
                               empty_width:-(area_size + empty_width)].shape)
        get_random_distribution(mat_type_area=mat_type_area,
                                num_block_resource=num_block_resource,
                                type_item=t)
        mat_type_all[:area_size, area_size +
                     empty_width:-(area_size + empty_width)] = mat_type_area
    elif t == 'sand':
        ...
        mat_type_area = np.zeros(
            shape=mat_type_all[area_size +
                               empty_width:-(area_size +
                                             empty_width), :area_size].shape)
        get_random_distribution(mat_type_area=mat_type_area,
                                num_block_resource=num_block_resource,
                                type_item=t)
        mat_type_all[area_size +
                     empty_width:-(area_size +
                                   empty_width), :area_size] = mat_type_area
    elif t == 'peanut':
        mat_type_area = np.zeros(
            shape=mat_type_all[area_size +
                               empty_width:-(area_size + empty_width),
                               -area_size:].shape)
        get_random_distribution(mat_type_area=mat_type_area,
                                num_block_resource=num_block_resource,
                                type_item=t)
        mat_type_all[area_size + empty_width:-(area_size + empty_width),
                     -area_size:] = mat_type_area
    elif t == 'stone':
        mat_type_area = np.zeros(
            shape=mat_type_all[-area_size:, :area_size].shape)
        get_random_distribution(mat_type_area=mat_type_area,
...
mat_type_all_ = [[dict_idx_type[mat_type_all[i][j]] for j in range(map_size)]
...
dict_json = {
    ...
    mat_type_all_,
    "amount": [[
        dict_reserve[mat_type_all_[i][j]] if mat_type_all[i][j] else 0
